...
Run the faster-whisper command directly using a Python subroutine, remove the dependency on whisper-ctranslate2, and update ffmpeg's parallel methods.

使用python子程序直接呼運行faster-whisper, 去除對whisper-ctranslate2的依賴, 更新ffmpeg的並行方法.
...
```bash
python whisper_fill_up_subtitle.py -i input.mkv -t events.txt --output_wav "D:\wav" --ass --whisper-output-dir "D:\txt" --model-dir "C:\Users\user\.cache\whisper"
```
...
python whisper_fill_up_subtitle.py -i %input% -t %lines% --output_wav "%~dp1wav" --ass --whisper-output-dir "%~dp1txt" --model-dir "C:\Users\user\.cache\whisper"
...
## Important 注意
While using default setting ```--model large-v3``` and ```--max-parallel 2```, the VRAM usage peak is around 6GB, Cuda usage ~80% (5070Ti), please choose a suitable setting for yourself

當使用預設 ```--model large-v3```和 ```--max-parallel 2```, 顯存使用峰值約為6GB, Cuda使用率~80% (5070Ti), 請自行選擇合適的參數
## Requirements 依賴項
 - Python ≥ 3.9
